...
def display_data(data_df):
    ...
    coefficient = 0
    while choice == 'yes' and coefficient * 5 < len(data_df):
        first_idx = coefficient * 5
        if first_idx + 5 <= len(data_df):
            end_idx = first_idx + 5
        else:
            end_idx = first_idx + (len(data_df) - first_idx)
        print(data_df.iloc[first_idx:end_idx])
        choice = input('Do you wish to continue?:  (yes/no)').strip().lower()
        coefficient += 1
